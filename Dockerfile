...
LABEL org.opencontainers.image.description="Search Austrian consolidated federal & state law, court decisions, and the authentic Bundesgesetzblatt from the official RIS via MCP. Keyless. STDIO or Streamable HTTP."
...
RUN mkdir -p /var/log/ris-austria-mcp-server && chown -R bun:bun /var/log/ris-austria-mcp-server
...
ENV LOGS_DIR="/var/log/ris-austria-mcp-server"
