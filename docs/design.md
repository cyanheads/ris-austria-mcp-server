...
| `query` | string? | `Suchworte` | |
| `title` | string? | `Titel` | |
| `ministry` | string? | `EinbringendeStelle` | Submitting ministry. Accepts the abbreviation ("BMF", "BMWET") or a full designation; `EinbringendeStelle` is a phrase field, so the service sends the bare abbreviation (✓ BMWET → 69 review drafts). An unknown abbreviation is rejected locally (`unresolved_ministry`); an unknown full designation (contains whitespace) is sent unchanged (✓ "Bundesministerium für Wirtschaft, Energie und Tourismus" → 69). Full table: ris_list_reference topic ministries. |
