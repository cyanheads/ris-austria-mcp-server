...
| `page`, `page_size` | number? | `Seitennummer`, `DokumenteProSeite` | page_size ∈ {10, 20} → Ten/Twenty. Default 20. 50 and 100 are rejected at the schema — read further with `page` (see Design Decisions). |

Every optional free-text parameter on the search tools (`query`, `title`, `municipality`, `section_from/to`, `law_id`, `index` here, and their counterparts on the other four) must carry a non-whitespace character: a blank or whitespace-only value is rejected at the schema, and omitting the field is how a filter stays unset.
...
| `issuer` | string? | `EinbringendeStelle` (BgblAuth) / `Einbringer` (Vbl, exact match: Landeshauptmann/frau \| Landesregierung \| Amt der Landesregierung \| Sonstige Landesbehörden — ✓ Tirol 89 / 477 / 0 / 15 of 581) | Ministry/body, e.g. "BMKOES". Federal: ministry abbreviations resolve against the ministries table (✓ BMWET → 69), an unknown full designation is sent unchanged, an unknown abbreviation fails as `unresolved_ministry` — as drafts. |
...
| `unresolved_ministry` | ValidationError | a federal `issuer` that is an abbreviation-shaped value matching no row of the ministries table (message names the closest matches), or a ministry the table lists only for another issuer parameter (message names the parameters that accept it) — thrown by the request builder before any upstream call | "Pass a ministry abbreviation or full designation from ris_list_reference topic ministries whose "Accepted by" includes einbringende_stelle — the message names the closest matches when there are any." |
...
| `issuer` set (federal) | "issuer names the submitting ministry, and the ministry name at the time of promulgation counts ('BMK' until 2025, not its successor 'BMIMI'). Abbreviations and designations: ris_list_reference topic ministries." (no wildcard advice — an abbreviation-shaped token missing from the table, `BMK*` included, is rejected as `unresolved_ministry`) |
| `issuer` set (ordinance gazette) | "An ordinance-gazette issuer is an exact match — use one of the Einbringer values listed in the issuer description, spelled exactly, with no wildcard." (Vbl `Einbringer` is an ExactMatchSearchExpression; ✓ RIS refuses a wildcard in-band) |
...
| `query` | string? | `Suchworte` | |
| `title` | string? | `Titel` | |
| `ministry` | string? | `EinbringendeStelle` | Submitting ministry. Accepts the abbreviation ("BMF", "BMWET") or a full designation; `EinbringendeStelle` is a phrase field, so the service sends the bare abbreviation for any ministry in the table, passed either way (✓ BMWET → 69 review drafts; the designation "Bundesministerium für Wirtschaft, Energie und Tourismus" sent raw also → 69). An unknown abbreviation is rejected locally (`unresolved_ministry`); an unknown full designation (contains whitespace) is sent unchanged. Full table: ris_list_reference topic ministries. |
...
**Output** (per record): `document_number` (BEGUT_/REGV_-prefixed GUID ✓), `stage`, `title`, `short_title`, `ministry` (Einbringer/Organ), `review_deadline` (Begutachtungsfrist end, review_drafts), `decided` (Beschlussdatum, government_bills), `document_url` (RIS web view), `content_urls` (Xml/Html/Rtf/Pdf of the main document ✓), `materials` (the companion documents filed with the draft — `type` `Material` \| `Letter` \| `Attachment`, a free-text `name`, and one `url`; ✓ 23,483 companions across the full live Begut + RegV corpus — 20,654 of them Xml/Html/Pdf with Rtf on 61%, and 2,829 (12.0%) Pdf or Pdf/Rtf with no Html at all, 2,820 of those Begut covering letters. The record carries the Html one, or the Pdf where RIS files no Html: `ris_get_document` validates a companion URL's extension and then discards it, rebuilding the rendition from its own `format` input, so every spelling of the URL is the same handle and carrying the set duplicated what `format` already serves. A Pdf-only companion has no text rendition — RIS 404s a rendition it does not list — so its URL is a download, not a `ris_get_document` handle. `type` is the reliable discriminator — the same content is filed as "Vorblatt und WFA", "Vorblatt+WFA", or "WFA". The `EmbeddedAttachment` inline formula images, ~55 per record, are excluded). Enrichment: totals/paging.
...
| `unresolved_ministry` | ValidationError | an abbreviation-shaped `ministry` matching no row of the ministries table (message names the closest matches), or a ministry the table lists only for another issuer parameter (message names the parameters that accept it) — thrown by the request builder before any upstream call | "Pass a ministry abbreviation or full designation from ris_list_reference topic ministries whose "Accepted by" includes einbringende_stelle — the message names the closest matches when there are any." |
| `invalid_query` | ValidationError | a page past the end and the RIS Client-error passthrough, as legislation | "For a page past the end, request a lower page, starting from 1. Otherwise correct the parameter named in the message. Ministry codes: ris_list_reference topic ministries." |
| `upstream_error` | ServiceUnavailable (retryable) | as legislation | as legislation |
...
| `issuer` | string? | `Urheber` (Avsv — exact, case-insensitive string, no wildcards; a carrier abbreviation such as ÖGK/DVSV/SVS/BVAEB/AUVA/PVA expands to the full designation, ✓ "ÖGK" → 0 raw vs "Österreichische Gesundheitskasse (ÖGK)" → 80; any other value, `<ABBR> Gesamtvertrag` included, is sent as written) / `Bundesministerium` (Erlaesse — full designation, abbreviation expanded as in drafts) / `Einbringer` (Mrp — exact `ABBR (Name)` composite, abbreviation expanded) | social_insurance, ministerial_decrees, council_minutes |
...
| `changed_since` | enum? | `ImRisSeit` | all except social_insurance and veterinary — Avsv/Avn accept `ImRisSeit` (a bogus value fails schema validation) but ignore every valid one (✓ 4,715 / 707 for none, one week, and one year alike, 2026-09-24), so it is rejected there, by the handler and by the request builder |
| `sort_by`, `sort_direction`, `page`, `page_size` | | `Sortierung.*`, paging | all; sort_by: `published` \| `number` where the app has the column |

**Output** (per record): `document_number` (AVSV_2026_0040, AVN_…, KMGER_…, PRUEF_…, SPG_…, ERL_BMJ_…, MRP_20260701_59 ✓ all observed), `collection`, `title`/`summary`, `number`, `published`/`session_date`, `issuer` (Organ/Einbringer), `norms_cited` (where present), `binding` (`authentic` for the five authentic collections ✓ Authentisch DataType observed \| `administrative_directive` for decrees \| `preparatory` for council minutes — per the canonical seven-label list in Design Decisions), `authentic_pdf_url` (where present), `document_url` (RIS web view — the only browsable surface for the PDF-only council minutes and for decrees), `content_urls`. Enrichment: totals/paging.

**Errors:** `collection_filter_mismatch` (ValidationError, local — a param outside its "Valid for" set, message names the pair; recovery routes to this table via ris_list_reference topic collections — except `changed_since` on social_insurance/veterinary, whose recovery names `published_from` for publication recency and `ris_track_changes` with `application: "Avsv"`/`"Avn"` for change recency, both ✓ run as written), `unresolved_ministry` (ValidationError, request builder — a ministerial_decrees/council_minutes `issuer` that is an unknown abbreviation, a ministry the table lists only for another issuer parameter (✓ `BMEUV` and `BMFFIM` on ministerial_decrees — the message names `Erlaesse Bundesministerium` and the parameters that do accept them, where it used to call each an unknown ministry and offer it as its own closest match, [#45](https://github.com/cyanheads/ris-austria-mcp-server/issues/45)), or an abbreviation with several designations this collection cannot tell apart; recovery routes to ris_list_reference topic ministries), `upstream_error` as legislation. `invalid_query` covers a page past the end and the RIS Client-error passthrough as legislation, plus the local rejections — a `plan_state` outside regional plans, and a `sort_by` value the collection has no column for, the last stated over the `collection` value the caller sent and naming the values it does sort by. Recovery: "For a page past the end, request a lower page, starting from 1. Otherwise correct the parameter named in the message, or drop it if this collection does not carry it. Collections and their issuers: ris_list_reference topic collections or issuing_bodies."

**Zero hits:** "0 documents in {collection}." + (`norm` set) "norm must match RIS's cited-norm format — copy from a result's norms_cited." + (`issuer` set) "issuer must match the RIS designation — ministry and social-insurance carrier abbreviations are expanded, any other value is matched exactly as written; issuers: ris_list_reference topic issuing_bodies or ministries." + (`collection: court_rules`) "KmGer currently carries LVwG Tirol and Vorarlberg rules only."
...
2. `document_url` — a `/Dokumente/…` URL passed through from a search result, on `https://www.ris.bka.gv.at` or `https://ogd.ris.bka.gv.at`. Validated: origin + path-prefix allowlist, nothing else fetched (SSRF guard). The origin list is exact — those two, plus the configured `RIS_CONTENT_BASE_URL` origin, which is `www.` by default — so another `*.bka.gv.at` host, a lookalike, a port, or `http://` is still `unsupported_url`. ✓ The OGD API now emits every `content_urls` and draft `materials[].url` on `ogd.` (all 457 on a 10-hit Begut page, 2026-09-24), and both hosts serve the same file byte for byte ([#43](https://github.com/cyanheads/ris-austria-mcp-server/issues/43)). The server still constructs `content_urls` on the configured host; a passed URL's origin is only checked, never carried into the fetch. The trailing filename must address one of exactly two things, or the URL is refused: the main-document rendition `{documentNumber}.{ext}`, or a companion document from a `ris_search_drafts` record's `materials[].url`. RIS files a companion under one of three filename shapes and all three are accepted whole — ✓ every one of the 23,483 companion URLs across the full live Begut + RegV corpus (7,185 records, every page of both applications, drafts from 2003 through 2026) parses, against 4,763 before [#35](https://github.com/cyanheads/ris-austria-mcp-server/issues/35):

   | shape | count | records |
   |:---|---:|:---|
   | `{Materialien\|Schreiben\|Anlagen}_{NNNN}_{UUID}` | 4,763 | 2026 only |
   | `COO_{n}_{n}_{n}_{n}` (Fabasoft object address — the leading group is a store id, not a year) | 18,719 | every record before 2026 |
   | `{NNNN}_{UUID}` | 1 | one RegV record |

   Each shape is an anchored alternative over `[0-9A-F_]` and the three literal prefixes, never a prefix or substring match — ✗ that distinction is load-bearing: RIS names the per-record inline formula images `Material-COO_…` and `Anlage-COO_…`, ~55 on every draft record, and a `COO_` substring test would address all of them. The extension is validated against the rendition set and then discarded: a companion is fetched inside the parent document's folder under the requested `format`, so any one of its rendition URLs addresses all of them (✓ the Html URL of `BEGUT_E811DCEF…`'s Erläuterungen returns 66,195 B of markdown, 166,836 B of html, and the same 77,536 B of Nutzdaten XML as fetching its `.xml` URL directly), and `content_urls` carries the companion's own Xml/Html/Pdf; there is no `.pdfsig` for one. Companion filenames are opaque and per-record — the URL is the only handle, which is why `document_number` cannot reach them, and why a refusal names its own cause (unrenderable extension · path below the folder · unrecognized filename) instead of telling the caller to re-copy a URL they copied.

   A companion RIS files without a text rendition — 2,829 of the 23,483, PDF or PDF/RTF only, 2,820 of them Begut covering letters — 404s under `markdown`, `html`, and `xml`, since RIS serves only the renditions it lists — and the Xml/Html entries `content_urls` constructs for it 404 the same way, leaving `content_urls.pdf` as the one that resolves. That is a `document_not_found` carrying a companion-specific recovery (the rendition is unpublished; re-call with `urls_only` and fetch `content_urls.pdf`), not the `format_unavailable` notice a PDF-only *application* gets: availability there is a property of the application and known before the fetch, while here it is per-companion and knowable only from the search result that produced the URL.

`format`: `markdown` (default — fetch HTML, strip boilerplate CSS/layout, convert; the reading format) | `html` (raw) | `xml` (RIS Nutzdaten schema) | `urls_only` (no fetch — all format links + authentic PDF). `html` and `xml` over the 40,000-byte budget return `kind: link` with no text (see Output) — in practice every `html` call, since each HTML rendition opens with a 40,566–69,758 B stylesheet header.
...
**Output:** `kind` (`full` | `outline` | `link`), `text` (unless urls_only/unavailable/outline/link), `format`, `byte_size`, `content_urls` (always, all DataTypes incl. `Authentisch` when present), `binding_status` (see Design Decisions › *Binding status*): `authentic` \| `consolidated_informational` \| `historical_record` \| `decision` \| `preparatory` \| `administrative_directive` \| `translation`, echoed identifiers. When the markdown text overflows the 40,000-byte budget the response becomes a `kind: outline` arm instead of truncating — `sections` lists the document's addressable entries (name + byte size) with `truncated: true`, and an enrichment `notice` names the re-call; a follow-up call passing `sections:[…]` returns just those. Entries are the document's §/Artikel/Anlage units where it carries at least two of them, and contiguous `Part n of N` byte windows where it carries none.

Raw `html`/`xml` are never sliced — a mid-document slice of either is not well-formed, the HTML body and 24 of 27 sampled XML renditions are a single line, and `windowDocument` cuts only at line breaks. The same 40,000-byte budget bounds them, measured on `byte_size`. At or under it the rendition returns whole (`kind: full`), and `content[]` carries it inside the framework's `markdown().codeBlock(text, format)`: the fence is one backtick longer than the longest run in the text, so the markup reaches the client verbatim and can neither render nor close the block early. Over it the result is `kind: link`: no `text` on either surface, `truncated: true`, the full `byte_size`, `content_urls`, and a notice naming `content_urls.<format>` (the whole artifact in one GET, byte-identical to what the tool used to inline) and `format: "markdown"` for readable text. A `sections` selector on a raw rendition is ignored with a notice either way. ✓ Measured 2026-09-24 across 27 documents in 13 applications: all 27 HTML renditions and 13 of 27 XML renditions are over the budget, up to 1,178,922 B of HTML (2.4 MB serialized) for `BGBLA_2025_I_100`, which now returns a result of about 2 KB.

Decision: one budget for both paths, not a second, larger raw budget. `OUTLINE_BUDGET_BYTES` already means "one result's worth", and HTML's reason to exist is fidelity to the published artifact, which `content_urls.html` delivers exactly. Decision: `link` is a new `kind` rather than a reuse of `outline` (which promises a `sections` roster to re-call with) or of `full` with `truncated` (which contradicts "the complete response"). Adding the member is a breaking change for consumers that switch exhaustively on `kind`.

The budget is sized against what one result can carry into a working context, not against what counts as a pathological document (✓ re-measured 2026-07-31 across 61 live renditions, cross-checked against a further 69): consolidated-law sections render at 2–3 KB and drafts and bills at 6–36 KB, while the multi-act bills, long English translations, and larger gazette issues that motivated the outline run 71 KB–199 KB and route through it. It was previously 500,000 — calibrated before the screen-reader strip roughly halved every rendered byte count, after which the bills it was written for came back whole.

It is also the window size, since a window is by definition one result's worth. `outlineOnOverflow` short-circuits below two entries and fires on whatever the server's `extract` callback returns, so one roster function decides what is addressable — `segmentDocument` where the markdown carries at least two §/Artikel/Anlage headings, `windowDocument` where it carries none — and both the outline and the `sections:[…]` selector read it, which is what makes the advertised roster equal the resolvable one. Without it a heading-free rendition returned whole however large, ✓ measured at 399,972 B on a Bvwg decision.

Windows are near-even slices snapped to line boundaries, `ceil(bytes / budget)` of them where that fits, named and listed in document order. Even rather than fill-the-budget: filling leaves a runt tail (a 40,440 B decision splits 39,602 + 838, a round-trip for a scrap) where an even split gives 20,196 + 20,244.

The budget is a ceiling, held by two rules. A window never takes a line that would carry it past the budget, so snapping to the next line break cannot overshoot; and when the tail still lands over, the count rises until every window fits — ✓ a 594,555 B Erv translation takes 16 windows where `ceil` alone gives 15 and overshoots. The exception is a line longer than the budget, which is never split — the cut would land where a caller cannot see it — so it sits alone in a window that goes over, and the count stops rising there. Without that stop the retry has no fixed point and climbs to one window per line: ✓ a 90 KB document carrying one 40 KB line listed 759 windows, 758 of them 66 B. ✓ Verified across 300 generated documents plus 2.6 MB / 200,000-line shapes: window bytes sum to `byte_size` exactly, the windows concatenate back to the source, and no multi-line window exceeds the budget.

Two properties a caller has to be told about, and the notice and the `sections` description do: a cut is a line break, not a sentence or § boundary, so a window can open mid-clause and RIS's table-heavy bodies orphan rows from their header row; and windows are a reading sequence, so `outlineDocument` restores document order after `outlineOnOverflow`'s largest-first sort (which has no opt-out) — a caller walking `sections` in array order would otherwise read the document scrambled. Section rosters keep that sort: their names say what each entry is, so they read as a menu.

Entries are contiguous slices, so `sections:[…]` joins matched text with nothing between them — a multi-entry selection reassembles byte for byte and its `byte_size` is the sum of the advertised entries. ✓ `Avsv/AVSV_2026_0037`'s six windows retrieved in one call return exactly the 206,425 B the whole rendition carries.

✓ Measured 2026-07-31 across 426 live renditions in 25 applications: 34 crossed the budget, of which 18 carried no §/Artikel/Anlage headings and reached nothing before this — exactly 0 sections in every one, never 1. Window counts land between 2 and 6, small enough to read a whole decision in a bounded number of calls. The affected applications are the ones that render as continuous prose: `Bvwg` (7 of 20), `Gbk` (3), `Vfgh` (2), `Dsk` (2), `Avsv` (2), `Lvwg` (1), `Justiz` (1). Nothing under the budget is affected — 392 of the 426 return whole in one call.
...
| `unsupported_url` | ValidationError | `document_url` fails the origin + `/Dokumente/` path-prefix allowlist (exactly `www.` and `ogd.ris.bka.gv.at`) — thrown locally, nothing fetched | "Only /Dokumente/ URLs on www.ris.bka.gv.at or ogd.ris.bka.gv.at are fetchable — pass a URL exactly as returned in content_urls, or switch to document_number + application." |
...
|:--------|:------|:--------|
| `RisService` | RIS OGD REST v2.6 (`data.bka.gv.at`) + document content hosts (`www.ris.bka.gv.at`, `ogd.ris.bka.gv.at`) | all tools except `ris_list_reference` |
...
- Ministry-abbreviation expansion for exact-match issuer params ("BMF" → "BMF (Bundesministerium für Finanzen)"), from the static ministries table; social-insurance carrier abbreviations for Avsv `Urheber` ("ÖGK" → "Österreichische Gesundheitskasse (ÖGK)"), from the abbreviation held on the issuing-bodies rows
...
| `RIS_CONTENT_BASE_URL` | no | Default `https://www.ris.bka.gv.at` (the host rendition URLs are constructed on; `document_url` accepts its origin alongside `www.` and `ogd.ris.bka.gv.at`) |
...
- **Ministry and social-insurance carrier abbreviations are expanded server-side.** RIS issuer params are exact-match against full designations ("BMF (Bundesministerium für Finanzen)", "Österreichische Gesundheitskasse (ÖGK)") including historical names — hostile to agents. The service accepts the abbreviation, expands from the static table, and `ris_list_reference` topics `ministries` and `issuing_bodies` carry the inventory. An unknown ministry abbreviation is a local ValidationError (`unresolved_ministry`) naming near-misses, not a silent zero-hit. The input is matched against the whole table before it is narrowed to the target parameter, so a ministry the table knows but that parameter does not accept fails with a message that says so and names the parameters that do accept it; ✓ a known historical designation that parameter lacks returns 0 upstream (`EinbringendeStelle` = "Bundesministerium für soziale Sicherheit und Generationen" on Begut and BgblAuth), so rejecting it loses nothing. A value the table lacks that contains whitespace — a full designation — passes through unchanged, because a static table goes stale: the 2025 cabinet's ministries were once rejected locally while RIS held their documents. Social-insurance issuers go further and never reject: every value but a known carrier abbreviation passes through, since `Urheber` matches exactly and refuses wildcards, so pass-through can only miss, never widen.
...
- **A blank text filter is rejected, not read as omitted (#39).** RIS answers an empty filter as no filter and ignores a whitespace-only `Abschnitt.Von`/`Bis` or `Urheber` outright, so a blank the server passed through or dropped would return the unfiltered set the caller meant to narrow — and a blank `municipality` re-routed a municipal search to consolidated state law. Every optional free-text input on the five search tools therefore requires a non-whitespace character at the schema (one shared `filterText`, advertised as a JSON Schema `pattern`), and omission is the only way to leave a filter unset. `ris_get_document` is the exception: a blank addressing field cannot select a document, so it never widens anything and stays tolerated.
- **`page_size` stops at 20 (#40).** Every record renders in full on both `structuredContent` and `content[]`, so a 100-row page put 125–402 KB on the wire (worst sampled page per tool) and a 50-row page 69–202 KB, before any document was opened. RIS pages by page number over a fixed page size, so smaller pages keep `page`/`pageSize`/`totalCount`/`truncated` exact with no new continuation mechanism; a byte-budgeted prefix of a larger page would shift the echoed coordinates and skip rows for a caller paging at its original size. The default stays 20, and the service keeps all four RIS sizes.
...
Base: `https://data.bka.gv.at/ris/api/v2.6/{controller}` — GET, JSON responses (JSON-serialized XML: `@attr`/`#text` nodes, object-or-array lists). Health: `GET /Version` → `{"OgdSearchResult":{"Version":"2.6"}}` ✓. Content host: `https://www.ris.bka.gv.at/Dokumente/{segment}/{DOKNR}/{DOKNR}.{xml|html|rtf|pdf}` (+ `.pdfsig` as `Authentisch`); the same tree is served on `https://ogd.ris.bka.gv.at`, the host search results now carry.
...
**Paging/envelope params:** `Applikation` (required, selects the application within the controller), `DokumenteProSeite` ∈ {Ten, Twenty, Fifty, OneHundred} (default Twenty ✓), `Seitennummer` (1-based), `ImRisSeit` ∈ {EinerWoche, ZweiWochen, EinemMonat, DreiMonaten, SechsMonaten, EinemJahr} — filters on the RIS change date (✓ Erlaesse one year 32 = History feed 32). Not on the gazette-law applications, and accepted but ignored on Avsv and Avn (✓ 2026-09-24, whole collection returned for every interval). Those applications offer `Kundmachung.Periode` with the same value set, but it buckets promulgation date, not change date (✓ Avn one year: Periode 43 vs History 55), so it is not mapped — `published_from/to` states promulgation recency exactly.
...
| BgblAuth | `Bgblnummer` ✓, `KundmachungsdatumVon/Bis` ✓ (≡ `Kundmachung.Von/Bis` ✓), `Teil.SucheInTeil1/2/3`, `Typ.SucheIn{Gesetzen,Verordnungen,Kundmachungen,Sonstiges}`, `EinbringendeStelle` (ministry phrase — an unlisted value matches nothing rather than failing validation) |
...
| Begut | `InBegutachtungAm` ✓ (→ 13 open reviews), `EinbringendeStelle` (ministry phrase), `ImRisSeit`, Sortierung (Kurztitel/EinbringendeStelle/EndeBegutachtungsfrist) |
...
| Avsv | `Urheber` (exact, case-insensitive string, wildcards refused — the full carrier designation, ✓ 56 distinct values across all 4,715 notices), `Avsvnummer`, `Dokumentart`, `Kundmachung.Von/Bis` or `.Periode`; `ImRisSeit` accepted but ignored |
...
| Avn | `Avnnummer`, `Typ.SucheIn{Kundmachungen, VeroeffentlichungenAufGrundVEVO, SonstigeVeroeffentlichungen}`, `Kundmachung.Von/Bis`, `FassungVom`, `Geschaeftszahl`, `Norm`; `ImRisSeit` accepted but ignored |
...
| Mrp | `Einbringer` (exact `ABBR (Name)` ministry composite — a free string upstream, an unlisted value matches nothing), `Sitzungsdatum.Von/Bis`, `Sitzungsnummer`, `Gesetzgebungsperiode` ✓ (XXVII → 235) — Pdf-only ✓ (no Titel) |
| Erlaesse | `Bundesministerium` (exact full designation, historical included — a free string upstream, an unlisted value matches nothing), `Abteilung`, `Fundstelle`, `Geschaeftszahl`, `Norm` ✓ (DSG → 10), `VonInkrafttretensdatum`/`BisInkrafttretensdatum`, `FassungVom` |
