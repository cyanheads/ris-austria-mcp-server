...
**Output** (per record): `document_number` (BEGUT_/REGV_-prefixed GUID ✓), `stage`, `title`, `short_title`, `ministry` (Einbringer/Organ), `review_deadline` (Begutachtungsfrist end, review_drafts), `decided` (Beschlussdatum, government_bills), `document_url` (RIS web view), `content_urls` (Xml/Html/Rtf/Pdf of the main document ✓), `materials` (the companion documents filed with the draft — `type` `Material` \| `Letter` \| `Attachment`, a free-text `name`, and the companion's own `urls`; ✓ observed across 200 live Begut/RegV records as `Materialien_`/`Schreiben_`/`Anlagen_` files, always Xml/Html/Pdf and Rtf on 41% of them. `type` is the reliable discriminator — the same content is filed as "Vorblatt und WFA", "Vorblatt+WFA", or "WFA". The `EmbeddedAttachment` inline formula images, ~55 per record, are excluded). Enrichment: totals/paging.
...
2. `document_url` — a `https://www.ris.bka.gv.at/Dokumente/…` URL passed through from a search result. Validated: host + path-prefix allowlist, nothing else fetched (SSRF guard). The trailing filename must address one of exactly two things, or the URL is refused: the main-document rendition `{documentNumber}.{ext}`, or a companion document from a `ris_search_drafts` record's `materials[].urls` — `{Materialien|Schreiben|Anlagen}_{NNNN}_{UUID}.{ext}` (✓ the shape of all 2,436 companion URLs across 200 live Begut/RegV records). A companion is fetched inside the parent document's folder, so the requested `format` still picks the rendition and `content_urls` carries the companion's own Xml/Html/Pdf; there is no `.pdfsig` for one. Companion filenames are opaque and per-record — the URL is the only handle, which is why `document_number` cannot reach them.
